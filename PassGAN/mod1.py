...
def ResBlock(name, inputs, dim):
    ...
    output = lib.ops.con1.Conv1D(name+'.1', dim, dim, 5, output)
    # print("After conv:", output)
    output = tf.nn.relu(output)
    output = lib.ops.con1.Conv1D(name+'.2', dim, dim, 5, output)
...
def Generator(n_samples, seq_len, layer_dim, output_dim, prev_outputs=None):
    ...
    output = lib.ops.l2.Linear('Generator.Input', 128, seq_len * layer_dim, output)
    ...
    output = lib.ops.con1.Conv1D('Generator.Output', layer_dim, output_dim, 1, output)
    output = softmax(output, output_dim)
    return output

def Discriminator(inputs, seq_len, layer_dim, input_dim):
    output = inputs
    output = lib.ops.con1.Conv1D('Discriminator.Input', input_dim, layer_dim, 1, output)
    ...
    output = tf.reshape(output, [-1, seq_len * layer_dim])
    output = lib.ops.l2.Linear('Discriminator.Output', seq_len * layer_dim, 1, output)
    return output
